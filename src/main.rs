...
fn add_stuff(space: &mut space::Space){
    space.add(shape::Shape {//todo move these space.add calls to a seperate function! this is bloating main
        shape_type: shape::ShapeType::Circle(0.2),
        position: vec2(0.5, 0.5), // centered
    });

    space.add(shape::Shape {
        shape_type: shape::ShapeType::Triangle(vec2(0.7,0.8),vec2(0.8,0.9),vec2(0.6,0.8)),
        position: vec2(0.5, 0.5), // centered
    });
}
